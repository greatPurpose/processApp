; BPUI.DragSort = function (elements)
{
    ...
    $(container).children().off("mousedown").mousedown(function (e)
    {
        ...
        // 添加虚线框
        _this.before('<div id="bpui_dragsort_dasheddiv"></div>');
        var wid = $("#bpui_dragsort_dasheddiv");
        ...
        $(document).mousemove(function (e)
        {
            ...
            $(container).children().not(_this).not(wid).each(function (i)
            {
                ...
                {
                    if (!obj.next("#bpui_dragsort_dasheddiv").length)
                    {
        ...
        $(document).mouseup(function ()
        {
            ...
            $(container).each(function ()
            {
                ...
                {
                    $("<div></div>").appendTo(this).attr("class", "bpui_dragsort_block").css({ "height": 100 });
                } else if (len == 2 && obj.is(".bpui_dragsort_block"))
                {
                    $(this).children(".bpui_dragsort_block").remove();
                }
            });
